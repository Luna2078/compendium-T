...
function relatorio(f: Ficha, titulo: string): string[] {
  ...
  L.push(`PV ${f.pv.atual}/${f.pv.max}${f.pv.temporario ? ` (+${f.pv.temporario} temp = ${f.pv.max + f.pv.temporario})` : ""}   PM ${f.pm.disponivel}/${f.pm.max}   Defesa ${f.defesa}   Deslocamento ${f.deslocamento}m`);
  L.push("");
